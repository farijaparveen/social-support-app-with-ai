...
export class Step2FamilyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      dependents: [saved.dependents ?? 0, [Validators.required, Validators.min(0), Validators.max(20)]],
      employmentStatus: [saved.employmentStatus || '', Validators.required],
      monthlyIncome: [saved.monthlyIncome ?? '', [Validators.required, Validators.min(0)]],
